    ...
    private Request followUpRequest(Response response, Route route) throws IOException {
        if (response != null) {
            ...
            if (code == 307 || code == 308) {
                if (!method.equals("GET") && !method.equals(c.METHOD_HEAD)) {
                    return null;
                }
            } else if (code == 401) {
                ...
                } else if ((response.priorResponse() == null || response.priorResponse().code() != 503) && retryAfter(response, ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED) == 0) {
                    return response.request();
                } else {
                    return null;
                }
            ...
            String header = response.header(c.HEADER_LOCATION);
            if (header == null) {
                return null;
            }
            ...
            if (HttpMethod.permitsRequestBody(method)) {
                ...
                if (!redirectsWithBody) {
                    ...
                    newBuilder.removeHeader(c.HEADER_CONTENT_TYPE);
                }
            }
            if (!sameConnection(response, resolve)) {
                newBuilder.removeHeader(c.HEADER_AUTHORIZATION);
            }
