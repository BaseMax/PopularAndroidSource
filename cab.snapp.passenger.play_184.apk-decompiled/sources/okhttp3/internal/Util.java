...
public final class Util {
    ...
    public static boolean skipAll(v vVar, int i, TimeUnit timeUnit) throws IOException {
        long nanoTime = System.nanoTime();
        long deadlineNanoTime = vVar.timeout().hasDeadline() ? vVar.timeout().deadlineNanoTime() - nanoTime : Long.MAX_VALUE;
        vVar.timeout().deadlineNanoTime(Math.min(deadlineNanoTime, timeUnit.toNanos((long) i)) + nanoTime);
        try {
            c cVar = new c();
            while (vVar.read(cVar, PlaybackStateCompat.ACTION_PLAY_FROM_URI) != -1) {
                cVar.clear();
            }
    ...
    public static Charset bomAwareCharset(e eVar, Charset charset) throws IOException {
        if (eVar.rangeEquals(0, UTF_8_BOM)) {
            eVar.skip((long) UTF_8_BOM.size());
            return UTF_8;
        } else if (eVar.rangeEquals(0, UTF_16_BE_BOM)) {
            eVar.skip((long) UTF_16_BE_BOM.size());
            return UTF_16_BE;
        } else if (eVar.rangeEquals(0, UTF_16_LE_BOM)) {
            eVar.skip((long) UTF_16_LE_BOM.size());
            return UTF_16_LE;
        } else if (eVar.rangeEquals(0, UTF_32_BE_BOM)) {
            eVar.skip((long) UTF_32_BE_BOM.size());
            return UTF_32_BE;
        } else if (!eVar.rangeEquals(0, UTF_32_LE_BOM)) {
            return charset;
        } else {
            eVar.skip((long) UTF_32_LE_BOM.size());
            return UTF_32_LE;
        }
    }
    ...
    private static String inet6AddressToAscii(byte[] bArr) {
        ...
        while (i < bArr.length) {
            if (i == i3) {
                ...
                cVar.writeHexadecimalUnsignedLong((long) (((bArr[i] & com.pusher.java_websocket.drafts.c.END_OF_FRAME) << 8) | (bArr[i + 1] & com.pusher.java_websocket.drafts.c.END_OF_FRAME)));
                i += 2;
            }
        }
