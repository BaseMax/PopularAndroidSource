...
public final class Http1Codec implements HttpCodec {
    ...
    final class ChunkedSink implements u {
        ...
        private final j timeout = new j(Http1Codec.this.sink.timeout());
    ...
    public final ResponseBody openResponseBody(Response response) throws IOException {
        ...
        String header = response.header(io.fabric.sdk.android.services.network.c.HEADER_CONTENT_TYPE);
        if (!HttpHeaders.hasBody(response)) {
            return new RealResponseBody(header, 0, n.buffer(newFixedLengthSource(0)));
        }
        if ("chunked".equalsIgnoreCase(response.header("Transfer-Encoding"))) {
            return new RealResponseBody(header, -1, n.buffer(newChunkedSource(response.request().url())));
        }
        long contentLength = HttpHeaders.contentLength(response);
        if (contentLength != -1) {
            return new RealResponseBody(header, contentLength, n.buffer(newFixedLengthSource(contentLength)));
        }
        return new RealResponseBody(header, -1, n.buffer(newUnknownLengthSource()));
    }
    ...
    public final v newChunkedSource(HttpUrl httpUrl) throws IOException {
        if (this.state == 4) {
            this.state = 5;
